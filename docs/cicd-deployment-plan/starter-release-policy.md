...
이 문서는 `observability-spring-boot-starter`를 GitHub Packages로 게시할 때의 버전, 태그, 검증, 호환성 정책을 정리한다.
...
| registry | GitHub Packages Maven registry |
| publish workflow | `.github/workflows/publish-starter.yml` |
...
7. publish 완료 후 GitHub Packages consumer 검증은 별도 consumer 프로젝트에서 `read:packages` PAT로 수행한다.
...
- 검증 명령: starter check, `publishToMavenLocal`, local consumer smoke, GitHub Packages consumer smoke 여부.
