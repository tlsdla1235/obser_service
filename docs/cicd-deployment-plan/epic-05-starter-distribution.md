...
- `observability-spring-boot-starter`는 기존 루트 기본 좌표(`com.sst`, `0.1.0-SNAPSHOT`)를 쓰던 `java-library`였으므로,
  외부 공개용 별도 좌표(groupId/artifactId/version) + publish 설정이 필요하다.
...
| version | `0.1.0` |
| starter release tag | `starter-v0.1.0` |
| GitHub Packages URL | `https://maven.pkg.github.com/tlsdla1235/obser_service` |

Portal production CD는 `v*` tag를 사용하므로 starter publish는 `starter-v*` tag로 분리한다.
...
- **AC**
  - starter 모듈에 `maven-publish` 적용, 명확한 `groupId`/`artifactId`(`io.github.tlsdla1235.observation:observability-spring-boot-starter`) 확정.
...
- **AC**
  - `.github/workflows/publish-starter.yml`: 트리거는 starter 릴리스 태그(`starter-v*`) 또는 release publish이며, release tag도 `starter-v*`만 처리한다.
  - `publishing.repositories`에 GitHub Packages URL(`https://maven.pkg.github.com/<owner>/<repo>`), 인증은 `GITHUB_TOKEN`(`packages: write`).
  - workflow `permissions`에 `contents: read`, `packages: write`를 명시하고, publish job은 starter guard와 local publish check 통과 후 실행되게 한다.
...
- **AC**
  ...
  - 실제 GitHub Packages publish 후 검증 샘플은 로컬 Maven cache에 기대지 않도록 clean Gradle user home 또는 별도 임시 프로젝트에서 실행.
  - publish 전 로컬 검증은 별도 임시 consumer 프로젝트에서 `mavenLocal()` 기준으로 같은 공개 좌표를 수신해 수행한다.
- **의존:** S5.2, S5.3
...
이번 구현 컨텍스트에서는 실제 `starter-v0.1.0` tag push와 GitHub Packages publish를 승인 전 실행하지 않는다.
