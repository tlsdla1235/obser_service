...
- **metric source of truth를 `Prometheus query profile`에서 `ingest envelope`로 되돌린다**
  - 단, v0.6을 그대로 복원하지 않고, 더 좁고 더 엄격한 bounded payload로 다시 정의한다
- **멀티 인스턴스 p95 정확도를 위해 percentile 책임을 starter가 아니라 서버로 이동한다**
  - starter는 app-level / endpoint-level histogram bucket을 보낸다
  - 서버는 인스턴스 간 bucket을 병합해 app-level p95와 endpoint 비교 근거를 계산한다
...
4. **histogram bucket 병합 기반 read model 설계 경험**
 - instance-local raw percentile 대신 server-side merge를 통해 app-level p95를 복원하는 방식
...
- Micrometer 계측, histogram merge, 규칙 기반 triage를 직접 설계·구현한 작품
...
3. 사용자는 비동기 전송, 큐, 재시도, histogram merge를 몰라도 된다
4. metric은 많이 모으는 것보다 **낮은 cardinality로 안정적으로 모으는 것**이 중요하다
5. percentile은 인스턴스별 숫자를 예쁘게 합치지 못하므로, 가능하면 histogram bucket을 보내고 서버에서 병합한다
...
2. `운영 첫 화면은 request/error/p95/top endpoint 정도만 보여줘도 충분히 가치가 있다`
...
대표 느린 요청 / 대표 에러 요청 sample 1~3개와 `traceId`를 같이 보여주는 최소 drill-down

이 기능은 "더 깊은 root cause"가 아니라 "한 건만 더 좁혀보자" 수준으로만 연다.
...
- starter는 최종 p95 숫자를 source of truth로 보내지 않는다
- starter는 histogram bucket을 전송한다
- 서버가 인스턴스 간 bucket을 병합해 app-level p95를 계산한다

이 결정을 택한 이유는 다음과 같다.

- 인스턴스별 percentile 값은 합치기 어렵다
- bucket은 병합이 가능하다
- 멀티 인스턴스 정확도는 MVP에서도 중요하다
...
6. portal은 payload를 검증하고 idempotent 저장한다
7. read model이 app summary와 endpoint priority view를 계산한다
...
1. `starter 추가 -> first bucket 수신 -> app alive 표시`
2. `request/error/p95 숫자 표시`
...
- 멀티 인스턴스 p95가 starter-local percentile보다 더 일관되게 계산된다
...
- starter-local p95를 source of truth로 쓰지 않는다
- histogram bucket merge 범위를 HTTP 서버 지표에 한정한다
- 범용 분포 엔진으로 확대하지 않는다
...
2. metric source of truth는 ingest envelope이다
3. p95는 starter가 아니라 서버가 histogram merge로 계산한다
...
- `v0.8`은 starter-first 설치 경험을 위해 bounded ingest contract를 다시 source of truth로 채택한다
