...
def main():
    ...
    global_custom_dir = Path(__file__).resolve().parent.parent / "custom"
    team = load_toml(global_custom_dir / f"{skill_name}.toml")
    user = load_toml(global_custom_dir / f"{skill_name}.user.toml")
    if project_root:
        custom_dir = project_root / "_bmad" / "custom"
        if custom_dir.exists():
            team = deep_merge(team, load_toml(custom_dir / f"{skill_name}.toml"))
            user = deep_merge(user, load_toml(custom_dir / f"{skill_name}.user.toml"))
