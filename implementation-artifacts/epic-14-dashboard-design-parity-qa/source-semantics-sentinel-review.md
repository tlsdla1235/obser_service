...
- 14.4: wide modal visual sentinel for `1120px`-class width, sticky header, normalized endpoint table overflow, and no narrow Sheet for live/snapshot detail. 14.4 implementation update: `frontend/scripts/read-model-contract-guard.ts` now statically verifies wide `DialogContent`, sticky `DialogHeader`, modal body section order from `InstanceContextNote` through `NormalizedEndpointEvidenceTable`, no extra `ContextHeader` panel before Application state reference, snapshot note no-override copy, `mode/source/window` info cells, and explicit `instance top-level state=없음`.
