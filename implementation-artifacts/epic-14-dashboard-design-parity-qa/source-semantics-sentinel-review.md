...
| Trend surface absence | `InstancePanels` static guard rejects `InstanceTrendView`, `Stored trend`, `SheetContent`, and `snapshotTrend`; trend read-model fixtures remain backend contract coverage only |
| Forbidden instance decision fields | `healthScore`, `rootCause`, `recoveryProof` negative fixtures/assertions |
| Instance wide modal grammar | 14.4 static sentinel verifies wide `DialogContent`, sticky header, no extra body header panel before Application state reference, modal section order, snapshot note override copy, and `instance top-level state=없음` |
...
- 14.3: retention expired/source absence browser fixture that proves no live/current fallback CTA appears in Snapshot detail and date/slot error state. 14.3 implementation update: `frontend/scripts/read-model-contract-guard.ts` now statically asserts Snapshot/History retention summary copy, 14-day retention marker query wiring, selected summary fields, secondary marker order, Snapshot detail top source flags, and forbidden raw explorer/fallback terms in Snapshot history/detail components. It also executes `snapshotRetentionDayKeys`, `snapshotSlotDayKey`, `snapshotSlotIndexFromWindowEndUtc`, and `snapshotSlotTimeLabel` boundary cases so midnight/end-label regressions are not string-only checks.
- 14.3: mobile slot grid visual sentinel note for 48-slot wrapping and clipped label checks. 14.3 implementation update: `SnapshotHistoryPanel` uses mobile 4-column, tablet 6-column, desktop 8-column slot grid with wrapping labels, and `deviation-log.md` records the mobile responsive adaptation as category 2. Browser evidence remains auth-blocked only without authenticated fixture.
- 14.4: wide modal visual sentinel for `1120px`-class width, sticky header, normalized endpoint table overflow, and no narrow Sheet/trend surface for live/snapshot detail. 14.4 implementation update: `frontend/scripts/read-model-contract-guard.ts` now statically verifies wide `DialogContent`, sticky `DialogHeader`, modal body section order from `InstanceContextNote` through `NormalizedEndpointEvidenceTable`, no extra `ContextHeader` panel before Application state reference, snapshot note no-override copy, `mode/source/window` info cells, explicit `instance top-level state=없음`, and absence of `InstanceTrendView`/`Stored trend`/`SheetContent`/`snapshotTrend` in `InstancePanels`.
- 14.4: authenticated full-path fixture/runbook candidate for `Project -> Application -> Dashboard -> Snapshot -> Instance -> retention expired`. 14.4 final QA update: no executable `.private/smoke-auth.env` token fixture was available, so browser evidence remains auth-blocked and the full authenticated path is still a coverage gap.
