...
| Instance wide modal | Browser: inaccessible without auth. Code: `DialogContent` width `min(1120px, calc(100vw - 2rem))`; stored trend remains Sheet. | `.modal`, `.modal-grid`, Instance Evidence Modal | Live/snapshot detail as wide modal, stored trend separate, section order preserved | Evidence gap. Code candidate aligns; visual clipping/table overflow needs screenshot | N/A - coverage gap. 후속 visual 차이는 1~4 또는 blocker로 재분류 | 14.4 |
