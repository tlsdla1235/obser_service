...
- **목록 행은 얇게 유지** (2026-06-11 D3 확정): `InstanceEntry` 목록 read model에 없는
  status/heartbeat/requests/slow/contribution은 행에서 직접 표시하지 않는다. 값은 per-instance
  `InstanceDashboardReadModel`을 여는 modal에서만 표시하며, 행 정보량 차이는 `SANCTIONED`로 추적한다.
...
| A3 | row: `status · heartbeat` (1760) | observationStatus + heartbeat 상대시각 | `dashboard.tsx:1623` `lastSeenAt`만 표시 | `SANCTIONED` | D3 확정: `InstanceEntry` 목록 read model에 status/heartbeat 없음. 행은 얇게 유지하고 modal에서 server 값을 표시 |
| A4 | row: `requests` 2칸 그리드 (1764) | instance 요청수 | 없음 | `SANCTIONED` | D3 확정: 목록 DTO 미포함. 행에서 N+1 호출/재계산하지 않음 |
| A5 | row: `slow >500ms` 2칸 그리드 (1765) | slow share | 없음 | `SANCTIONED` | D3 확정: 목록 DTO 미포함. modal `signals.red`에서만 표시 |
| A6 | row: `contribution` 배지 (1768) | contributing/supporting/attention/insufficient 배지 | 없음 (행 안내 카피만 있음, `dashboard.tsx:1625-1627`) | `SANCTIONED` | D3 확정: 목록 DTO 미포함. modal `applicationContribution`에서만 표시 |
...
> A3~A6는 SoT row와 production 목록 read model 사이의 차이를 의도적으로 허용한 행이다. 현재
> `InstanceEntry`(`read-model-types.ts:416-423`)는 `instanceId/instanceName/lastSeenAt/links.evidence`만 제공한다.
> backend/schema 무변경 원칙에 따라 frontend에서 행 단위 재계산이나 per-row dashboard fetch를 하지 않는다.
...
| D3 | SoT row는 status/heartbeat/requests/slow/contribution을 표시하지만 `InstanceEntry` 목록 read model에는 없음 | **확정 (2026-06-11): `SANCTIONED`.** 행은 얇게 유지하고, 값은 modal의 per-instance `InstanceDashboardReadModel`에서만 표시한다. | A3~A6를 `SANCTIONED`으로 고정. backend/schema 변경, frontend 재계산, 행별 N+1 fetch 금지 |
...
1. **D1/D2/D3/D4 확정** (§7) — 시계열 trend retire, B2 provenance 허용, row thin 허용, modal field는 server read model 상한으로 제한.
2. **Summary row** — 단일 "Open modal" 버튼만 유지. A3~A6는 `InstanceEntry` 계약 차이로 `SANCTIONED`.
...
| # | 충돌 | 선택지 | 영향 |
|---|---|---|---|
| D3 | SoT row는 status·heartbeat·requests·slow·contribution을 행에 표시하지만, 실제 `InstanceEntry`(application dashboard 목록 read model, `read-model-types.ts:416`)에는 `instanceId/instanceName/lastSeenAt/links.evidence`만 있다. 이 값들은 per-instance `InstanceDashboardReadModel`(모달 호출 시점)에만 존재. | **확정: 행은 얇게 유지**(현 구현). A3~A6는 모달에서만 보고, SoT row와 정보량 차이를 의도적 deviation으로 `SANCTIONED`한다. backend 목록 보강 story와 행별 N+1 호출은 MVP에서 하지 않는다. | A3~A6 최종 상태는 `SANCTIONED`. "표시만/백엔드 무변경" 원칙 유지 |
