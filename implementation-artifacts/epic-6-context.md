...
Account-project visibility와 project-scoped resource authorization은 Story 6.10에서 N:M membership으로 닫는다. MVP membership role은 `member` 단일 role이고 status는 `active`, `disabled`만 둔다. `GET /api/projects`는 authenticated account의 active membership project만 반환하며, `/api/projects/{projectId}/applications/**` resource API는 membership mismatch를 `404`로 fail-closed한다. Project creation 방식, project key 발급/회전/재발급, invite/team/org management, billing/tenant model은 open decision으로 남아 있다. Story 구현 중 public onboarding API, 로그인 직후 자동 project 생성, Create Project flow가 필요해 보이면 구현하지 말고 contract decision으로 올린다.
...
Project resource API는 Bearer service access token으로 인증된 account id와 active account-project membership을 함께 확인한다. Epic 5의 project/application/instance 관계 검증은 catalog path 정합성이고, Story 6.10의 membership은 account-project authorization이다. 두 경계 모두 mismatch 시 `404`를 사용해 project/application/snapshot 존재 여부 노출을 줄인다.
...
Story 6.1은 GitHub OAuth only account entry, Bearer token 기준, minimal setup guide, static dashboard asset boundary를 마련한다. Story 6.2는 `GET /api/projects` project navigation read model을 소비해 Project selection을 닫고, Story 6.3이 인증 fetch 가능한 Application List UI를 구현할 때 사용할 `links.applications` 경계를 보존해야 한다. Story 6.4 이후 화면은 Epic 5 dashboard/snapshot/history read model 계약을 그대로 렌더링해야 한다. Story 6.10은 이 화면 흐름을 다시 설계하지 않고, 같은 API surface를 authenticated account membership scope 안으로 제한한다.
