...
## SQS Buffered Ingest 계획과 성능 검증 방침

Epic 12에서는 ingest request path에서 DB bucket insert를 직접 수행하지 않고, 검증된 bucket message를 SQS에 넣은 뒤 Spring Boot portal 내부 worker가 PostgreSQL에 저장하는 전환을 계획합니다. Lambda consumer는 이번 범위의 구현 대상이 아니라 deferred option입니다.

성능 근거는 배포 환경에 direct baseline을 별도로 올려 비교하지 않고, 동일 코드베이스의 direct mode와 SQS mode를 local 또는 격리 benchmark profile에서 비교해 남깁니다. 같은 fixture, 같은 idempotency 분포, 같은 DB 초기 상태로 request latency, enqueue-to-persist lag, DB statement/round-trip count, batch writer throughput을 분리해 측정합니다.

배포 환경에는 의도한 SQS buffered ingest path만 올리고, direct-vs-SQS 성능 비교 대신 SQS queue, worker, DLQ, rollback config가 동작하는지 smoke verification으로 확인합니다. 로컬 benchmark 결과는 portfolio evidence와 상대 추세 근거로만 사용하며, production-grade load test나 autoscaling 성능 보증으로 주장하지 않습니다.
...
2. SQS buffered ingest 전환
   - 인스턴스별 ingest 요청을 매번 DB에 바로 쓰지 않고 SQS에 적재
   - Spring Boot portal 내부 worker가 bounded batch로 PostgreSQL에 저장
   - direct mode와 SQS mode의 성능 비교는 local/격리 benchmark에서 수행하고, 배포 환경은 SQS path smoke verification 중심으로 검증
   - 늦게 도착하는 ingest를 고려해 snapshot 생성 시점과 queue lag 의미를 분리하는 방향
