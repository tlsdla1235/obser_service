...
Epic 13 이후 구현 보조 기준은 최종 Dashboard Source of Truth 정렬 결과를 따른다. Project Entry와 Application List는 scope 선택/read-only scan surface이고, Application Dashboard가 primary first-screen이다. Snapshot detail/history는 stored dashboard read model 복원 및 30분 slot 탐색 surface이며, Instance Dashboard live/snapshot mode와 Instance Snapshot Trend는 서로 다른 evidence/projection surface다.
...
| `ProjectApplicationNavigationService` 후보 | `domain.catalog.service` 또는 `domain.dashboard.service`에서 Project Entry/Application List용 light summary read model 생성 |
| `InstanceEvidenceReadModelService` 후보 | `domain.instance.service` 또는 `domain.dashboard.service`에서 Instance Detail용 bounded evidence read model 생성 |
...
| `InstanceSnapshotTrendService` 후보 | `domain.history.service` 또는 `domain.instance.service`에서 `dashboard_snapshots.read_model_json`의 bounded instance summary를 특정 instance trend로 projection |
...
Operational event history와 instance snapshot trend MVP 후보는 `DashboardSnapshotRepository`를 재사용한다. 별도 `OperationalEventRepository`, `operational_events` table, raw instance timeseries table은 보류한다.
...
Starter route attribution allowlist는 `config` 영역의 starter configuration으로 선언하며, MVP namespace는 `observation.route-attribution.allowlist`다. 항목은 route template만 허용하고 query string, absolute URL, 실제 ID 값은 거부한다. Route normalization precedence와 fallback 허용 범위는 `planning-artifacts/contracts/route-attribution-policy.md`를 따른다.
...
| `scheduler` | cleanup, application별 30분 scheduled dashboard snapshot trigger | business semantics 직접 판단 |
...
- Redis queue 없이 portal scheduled service가 application별 30분 dashboard snapshot을 저장한다. Live dashboard query는 `accepted_metric_buckets` 기준 `recent_30_minutes` read model을 생성하고, snapshot detail은 stored `dashboard_snapshots.read_model_json`을 복원한다.
...
- starter route allowlist가 없고 framework route도 없을 때의 route attribution은 `planning-artifacts/contracts/route-attribution-policy.md`를 따른다. low-cardinality raw 후보에서 safe prefix를 확정할 수 있으면 bounded route로 접고, 그렇지 않으면 `UNKNOWN`으로 수렴한다.
- dashboard UI는 `read-model-contract` response를 그대로 렌더링하고, 복잡한 client-side 상태 관리는 두지 않는다.
- Project Entry와 Application List는 catalog/read model의 light summary만 렌더링하고 Application Dashboard 판단을 대체하지 않는다.
- Instance Snapshot Trend는 stored dashboard snapshot/read model projection으로 시작하고, 조회 비용이 커질 때만 snapshot-derived helper table을 검토한다.
...
- UI에서 instance snapshot trend point를 조합해 instance health score, lifecycle state, p95/p99, endpoint priority를 재계산
- PostgreSQL view/materialized view에 p95/state/rule 계산을 숨기는 구현
- 별도 `operational_events` table 또는 event repository
- raw instance timeseries table 또는 endpoint timeseries table
...
- instance trend controller 후보는 저장된 dashboard snapshot/read model의 bounded instance summary를 projection할 뿐 current state, health score, p95/p99, endpoint priority를 재계산하지 않음을 검증한다.
...
- UI는 instance trend를 표시하더라도 raw bucket explorer, arbitrary metric query, endpoint timeseries처럼 확장하지 않는다.
