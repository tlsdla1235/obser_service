...
Epic 5/6 구현 보조 기준은 최신 dashboard baseline을 따른다. Project Entry와 Application List는 scope 선택/read-only scan surface이고, Application Dashboard가 primary first-screen이다. Instance Detail은 evidence drill-down, Instance Snapshot Trend와 Snapshot/History는 stored dashboard snapshot/read model projection이다.
