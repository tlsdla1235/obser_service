...
4. 최종 기준은 “starter heartbeat는 control-plane/liveness signal이고, accepted bucket만 freshness/state/read-model source-of-truth이며, `localPercentiles`는 instance-local 30초 bucket percentile evidence”라는 문장이다.
...
- `localPercentiles.p95Ms`/`p99Ms`는 해당 starter가 해당 30초 bucket에서 관측한 instance-local percentile value다.
- n분 요약은 “n분 전체 요청의 p95/p99”가 아니라 “n분 동안 생성된 30초 bucket p95/p99 값들의 summary statistics”다.
- n분 요약에는 평균, 중앙값, 최솟값, 최댓값을 둘 수 있다. 여기서 n은 current window, baseline window, snapshot window 모두에 적용될 수 있다.
- 이 값들은 app/project/window rollup source-of-truth가 아니며, application state/freshness/rule/endpoint priority 계산에 쓰지 않는다.
- 그러나 instance detail에서는 `instance-local 30초 bucket p95/p99 요약`, `bucket p99 평균`, `bucket p99 중앙값`, `bucket p99 최댓값`처럼 명확히 라벨링하고 충분히 강조해서 보여줄 수 있다.
- app/project canonical p95/p99와 rule 판단은 계속 accepted bucket histogram merge 결과를 따른다.
...
5. **starter ingest schema에 instance p95/p99를 source of truth로 추가하는 것은 반대합니다.**  
   p95/p99는 merge 불가능성이 사용자 신뢰를 깨기 쉽습니다. “인스턴스 p95를 합쳐 앱 p95처럼 보이는” 순간 제품의 설명 가능성이 무너집니다.

6. **옵션은 A가 MVP 기준으로 가장 맞습니다.**  
   A: histogram은 계속 보내고, 서버가 instance histogram 기준으로 instance p95/p99를 계산합니다.  
   B는 나중에 auxiliary evidence로만 가능하지만 MVP에는 넣지 않는 게 좋습니다.  
   C는 반대입니다. histogram을 줄이고 p95/p99 중심으로 가면 app/endpoint 병합 신뢰성과 향후 분석 여지가 크게 약해집니다.
...
8. **long-window p99는 제품에서 조심스럽게 다뤄야 합니다.**  
   snapshot detail에는 저장 당시의 read model과 marker를 보여주고, 14일 전체 p99 같은 숫자는 대표 지표처럼 노출하지 않는 게 좋습니다. tail latency는 “해당 시점의 보조 증거”로만 표현하고, 긴 기간 비교는 spike/error marker의 발생 시점과 빈도로 풀어야 합니다.
...
제품적으로 가장 중요한 선은 이겁니다: starter는 histogram과 summary를 보내고, portal이 판단한다. UI는 판단 결과와 근거를 보여주되 재계산하지 않는다. p99는 유혹적인 숫자지만 MVP의 주인공이 아니라, 신뢰 가능한 판단을 보조하는 증거여야 합니다.
...
p95/p99 ingest schema는 저는 **C안에 가깝게** 가는 편이 맞다고 봅니다. 즉, 스타터가 p95/p99 숫자를 authoritative하게 보내는 구조는 피하고, **인스턴스별 cumulative histogram bucket을 보내되 서버가 병합/계산한 read model만 UI가 표시**하는 방향입니다. A안이 “스타터가 p95/p99 직접 전송”이라면 UX가 빠르게 좋아 보이지만 신뢰 모델이 깨집니다. B안이 “p95만 보내고 p99는 서버 계산” 같은 절충이라면 사용자가 왜 어떤 값은 로컬이고 어떤 값은 서버 판단인지 이해하기 어렵습니다. C안은 구현은 더 무겁지만 UX 언어가 가장 깨끗합니다: “포털이 같은 기준으로 계산한 값입니다.”

특히 p99는 화면에서 조심스럽게 다뤄야 합니다. p99를 큰 숫자 카드로 올리면 사용자는 즉시 SLA 지표처럼 받아들입니다. 현재 문서 방향대로라면 p99/tail latency는 **보조 증거**여야 하고, “긴 기간 평균 p99”처럼 보이면 안 됩니다. UI에서는 “tail signal”, “spike evidence”, “일시적 꼬리 지연 감지” 같은 표현이 낫고, 숫자보다 마커와 맥락이 먼저 와야 합니다.
...
저는 **A안: histogram은 유지하고 서버가 instance p95/p99를 계산**을 추천합니다.

Percentile 수학은 엄격해야 합니다.

- p95/p99는 평균낼 수 없습니다.
- instance p95들을 평균해서 application p95를 만들 수 없습니다.
- endpoint p95들을 평균해서 application p95를 만들 수도 없습니다.
- 올바른 집계는 동일 bucket boundary를 가진 histogram count를 합친 뒤 quantile을 다시 계산하는 방식입니다.

그래서 현재 방향인 “starter가 cumulative histogram buckets를 보내고 서버가 병합한다”가 맞습니다.

B안, 즉 starter가 instance p95/p99를 함께 보내는 방식은 보조 증거로도 위험합니다. 결국 두 개의 진실이 생깁니다. “starter p95는 800ms인데 portal p95는 1.2s” 같은 질문이 나오고, 그 순간 제품 신뢰가 깎입니다. 정말 필요하다면 디버그 전용 필드로 격리해야 하지만 MVP에는 넣지 않는 게 좋습니다.  
Superseded: 최신 기준에서는 `localPercentiles.p95Ms`/`p99Ms`를 instance-local 30초 bucket evidence로 수용한다. 이 값은 app/project canonical percentile이나 rule 판단의 source-of-truth가 아니다.

C안, histogram을 줄이고 p95/p99 중심으로 전환하는 방식은 피해야 합니다. 저장량은 줄지만 운영 제품의 핵심 자산을 버립니다. 나중에 application, endpoint, instance, environment 단위로 다시 묶어 볼 수 없고, p99도 더 부정확해집니다.
...
다만 p99는 조심해서 보여야 합니다. 낮은 traffic의 15분 p99는 “운영 상태”라기보다 “꼬리 지연의 참고 증거”입니다. 화면에서도 state 판정의 주역은 p95/error/spike concern이고, p99는 auxiliary evidence로 두는 게 맞습니다.
...
3. instance p95/p99는 starter payload에 추가하지 않는다.
4. 서버가 동일 histogram bucket을 병합해 application/endpoint/instance percentile을 계산한다.
...
Instance-level p95/p99 ingest는 **C안: starter가 instance histogram buckets만 보내고 portal이 instance/app/endpoint p95/p99를 계산**으로 고정하는 게 맞습니다. 기존 원칙과도 맞습니다. starter-local p95/p99는 source of truth가 아니고, p99 때문에 payload를 확장하지 않는다는 현재 문서를 깨면 안 됩니다.  
현재 기준에서는 이 문장을 “starter-local p95/p99를 app/project source-of-truth로 쓰지 않는다”로만 해석한다. `localPercentiles` 자체는 instance-local 30초 bucket evidence로 허용한다.
...
- histogram merge 기반 p95/p99 계산 테스트
- starter-local p95/p99 필드가 들어와도 무시하거나 schema validation에서 거절  
  - Superseded: 최신 기준에서는 `localPercentiles`를 거절하지 않고, `mergeable=false` instance-local evidence로 수용하되 app/project rollup input에서 제외한다.
...
- starter-local p95/p99를 source of truth로 삼지 않음
...
5. **히스토그램/p95/p99 정렬**  
   Starter는 buckets를 보냅니다. Server는 병합해 p95를 계산합니다. p99는 read model의 보조 지표일 수 있지만 starter payload 확장은 하지 않습니다.  
   Superseded: 최신 기준에서는 `localPercentiles` payload 확장을 instance-local 30초 bucket evidence로 허용한다. app/project canonical p95/p99와 rule 판단은 여전히 merged histogram 기반이다.
...
“starter-first direct ingest”는 유지. 단, “server computes p95” 근처에 “p99도 starter payload가 아니라 merged histogram 기반 read model의 보조 지표”를 추가합니다. “user should not need to know histogram merge”는 “UI와 사용자는 병합 방식을 몰라도 된다”로 명확히 합니다.
...
`api-surface.md`  
ingest buckets endpoint와 dashboard endpoint는 현 상태를 유지합니다. Candidate 영역에 heartbeat를 넣는다면 “candidate control-plane endpoint, not metric ingest”라고 표시합니다. Dashboard response 설명에는 “state, p95, p99, endpoint priority are server-computed”를 추가합니다.

`read-model-contract.md`  
hourly application snapshots 설명에 project/application/environment/instance 식별자를 명확히 둡니다. p99는 “auxiliary, merged-histogram-derived”로 고정합니다. “starter p99 payload expansion 없음”을 명시합니다.  
Superseded: 최신 기준에서는 `localPercentiles.p99Ms`를 instance-local bucket evidence로 받을 수 있다. 다만 snapshot/app/project canonical p99와 rule 판단은 histogram-derived boundary를 유지한다.
...
`ingest-envelope.md`  
`project key header`, `application`, `environment`, `instance`를 계층 설명으로 묶습니다. Endpoint histogram buckets는 “서버 병합 입력”이라고 설명하고 p95/p99 payload 필드는 추가하지 않습니다.
...
2. Docs state that starter sends histogram buckets, while server computes merged read models including p95 and auxiliary p99.
...
- **B: histogram bucket ingest**를 기본으로 채택. merge 가능한 구조라 app/project p95/p99를 계산할 수 있습니다.
- **C: histogram + instance-local p95/p99 보조 필드**는 가능하지만, 보조 필드는 “instance-local observed percentile”로 라벨링하고 rollup 입력으로 쓰지 않는다는 계약이 필요합니다.
...
- 후속 snapshot story는 저장 당시 read model, instance-local percentile summary, histogram-derived bounded tail evidence를 분리해 정의한다.
- 후속 `trendSlices`의 p95/p99는 p95/p99 값끼리 평균낸 값이 아니라 해당 subwindow의 histogram buckets를 merge한 뒤 다시 계산한 값이어야 한다.
...
개별 instance에 대해서는 starter가 각 30초 bucket에서 직접 관측한 p95/p99를 payload에 포함한다. 이 값은 app/project/window rollup의 source of truth가 아니라 **instance-local 30초 bucket percentile evidence**다. 사용자는 instance detail에서 정확한 starter-local p95/p99 값을 보고 싶어 하므로, 이 요구는 제품적으로 타당하다.
...
- `localPercentiles.p95Ms`와 `localPercentiles.p99Ms`는 해당 instance의 해당 30초 bucket에 대해서만 정확한 starter-local observed value다.
- `localPercentileSummary.p95.avgMs`와 `p99.avgMs`는 canonical n분 p95/p99가 아니라, current/baseline/snapshot window 안의 30초 bucket percentile 값들의 평균이다.
- `medianMs`, `minMs`, `maxMs`도 같은 bucket percentile point 집합에 대한 요약 통계다.
- 기본 평균은 30초 bucket별 동일 가중 산술 평균으로 문서화한다. request-count weighted average를 도입하려면 별도 필드와 라벨로 명시하고, “15분 p95/p99”처럼 부르지 않는다.
- instance detail은 이 avg/median/min/max를 크게 보여줄 수 있지만 label은 “instance-local 30초 bucket p95/p99 요약”, “bucket p99 평균”, “bucket p99 중앙값”, “bucket p99 최댓값”처럼 의미가 분명해야 한다.
- 피해야 할 라벨은 “15분 p99”, “current p99”, “canonical p99”, “application p99”, “서비스 p99”다.
- app/project p95/p99, canonical 15분 window p95/p99, rule source-of-truth는 histogram merge 결과를 유지한다.
...
6. 사용자 제시 옵션 기준으로는 **A: histogram 유지 + 서버가 instance/app/endpoint p95/p99 계산**이 권장된다.
7. starter ingest schema에는 app/project/window rollup source-of-truth 필드로 `instanceP95`/`instanceP99`를 추가하지 않는다. 대신 `localPercentiles` 같은 instance-local 30초 bucket percentile field는 명시적으로 허용한다.
8. p99는 app/project 판단에서는 auxiliary evidence이며 단독 degraded/down 판단, 단독 rank reason, 긴 기간 대표 지표가 아니다. Instance detail에서는 starter-local bucket p99 summary를 구체적인 값으로 강조해서 보여줄 수 있다.
...
10. snapshot에서 p99를 다룰 때는 단일 long-window 대표값이 아니라 histogram-derived subwindow trend, worst bucket/slice, bad bucket count, marker로 제한한다.
11. 개별 instance detail은 30초 bucket p95/p99 point들의 avg/median/min/max를 current/baseline/snapshot window별로 크게 보여줄 수 있다.
...
2. Sally와 Amelia가 응답에서 옵션 라벨을 다르게 사용했다. 둘 다 app/project rollup에는 histogram merge가 필요하다는 데 동의했다.
...
7. ingest schema에는 app/project/window rollup source로 쓰일 percentile value 필드를 추가하지 않는다. 다만 instance-local bucket percentile evidence로 `localPercentiles`를 추가한다.
8. server-side `HistogramMergeService`가 application, endpoint, instance scope의 canonical p95를 계산하고, sample guard 통과 시 p99를 auxiliary evidence로 계산한다.
9. project/application/instance navigation은 first-screen UX를 구체화하되, application dashboard를 첫 판단 화면으로 유지한다.
10. snapshot history는 후속 story에서 marker와 bounded tail summary 중심으로 표현한다. Story 4-0에서는 긴 시간 범위 단일 p99 금지와 histogram-derived bounded evidence 원칙만 고정한다.
11. instance detail dashboard는 starter-local 30초 bucket p95/p99 point들의 current/baseline n분 avg/median/min/max를 headline으로 보여준다.
...
3. **instance별 p95/p99를 ingest schema에 추가하는 것이 맞는가?**  
   app/project/window rollup source-of-truth 필드로는 추가하지 않는다. 그러나 개별 instance의 30초 bucket percentile evidence로 `localPercentiles.p95Ms`/`p99Ms`를 추가하는 것은 권장한다.

4. **histogram을 계속 유지해야 하는가? p95/p99는 어떤 역할인가?**  
   유지해야 한다. Histogram은 app/project/window canonical percentile과 rule 판단의 안전한 source다. Starter-local p95/p99는 개별 instance의 30초 bucket point와 n분 avg/median/min/max headline summary에 사용한다.

5. **시간 단위 snapshot persistence에서 p99/p95를 어떻게 표현해야 하는가?**  
   snapshot detail에는 저장 당시 window의 read model 값, instance-local bucket percentile summary, bounded tail summary를 표현한다. 긴 기간 summary에는 단일 p99를 대표값처럼 노출하지 않고 `trendSlices`, `worstBuckets`, `badBucketCount`, marker, bucket p95/p99 avg/median/min/max, baseline delta를 우선한다.
...
1. **Contract baseline first**
   - `ingest-envelope.md`: heartbeat와 bucket ingest의 경계를 추가하고 `localPercentiles`의 instance-local/non-mergeable 의미를 명시한다.
   - `histogram-merge.md`: app/project/window canonical percentile과 starter-local trend summary의 차이를 명시하고, p99 auxiliary/sample guard를 유지한다.
...
- Instance detail에서는 30초 bucket p95/p99 point들의 avg/median/min/max를 큰 headline card로 표시하되, “instance-local bucket p99 요약”처럼 non-canonical 의미를 라벨에 드러낸다.
- App/project dashboard에서는 p99를 큰 primary 판단 카드가 아니라 tail/spike auxiliary evidence로 표시한다.
...
- 9.3 p95 계산 책임에 app/project/window canonical percentile은 histogram merge로 계산하고, instance-local 30초 bucket p95/p99 point와 n분 avg/median/min/max summary는 starter-local evidence로 사용할 수 있다고 추가한다.
...
- instance read model은 server histogram merge 기반 canonical p95/p99와 starter-local bucket p95/p99 avg/median/min/max summary를 구분해서 담을 수 있다고 명시한다.
...
- Snapshot `read_model_json` 또는 bounded evidence 후보는 후속 story에서 확정하고, 이 정렬에서는 `localPercentileSummary`와 histogram-derived bounded tail evidence를 분리해야 한다는 원칙만 추가한다.
...
- p99는 snapshot detail auxiliary evidence, instance-local percentile summary, 후속 bounded tail evidence로만 남기고 long-range p99 rollup을 금지한다.
- “worst bucket”, “worst slice”, “bad bucket count”는 raw retention 범위에서 계산하거나 snapshot에 저장된 bounded summary에서만 표현한다고 제한한다.
- `trendSlices`의 p99는 percentile 값끼리 합친 값이 아니라 subwindow histogram merge 결과라고 명시한다.
...
- `schemaVersion: "1.0"` 또는 후속 schema에 `localPercentiles` 후보를 추가하되, scope는 `instance_bucket`, source는 `starter_local`, `mergeable=false`로 고정하고 n분 summary는 bucket percentile point들의 avg/median/min/max라고 명시한다.
