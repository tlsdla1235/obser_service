...
| P1 | Route Attribution B안 승인 후 Story 2.1/2.2/2.3과 contract/architecture/traceability가 같은 안전 조건을 말해야 했다. | `http.route`/framework route template 우선, raw path candidate의 allowlist 일시 입력, query 폐기, exact-one match, `UNKNOWN` 수렴 조건이 흩어지면 구현자가 fallback을 넓게 해석할 수 있다. | 2026-05-14 보정으로 Story 2.1/2.2/2.3, `metric-taxonomy.md`, `ingest-envelope.md`, `read-model-contract.md`, `insight-rules.md`, architecture 계열 문서, sprint plan, acceptance traceability에 B안 기준을 반영했다. |
...
| 2.2 Route Normalization and Low-Cardinality Guard | Ready after 2.1 | 2.1 observation input을 받아 normalized route와 allowed tag만 다음 단계로 넘긴다. | `http.route`를 최우선으로 사용하고, raw path candidate는 allowlist exact-one matching의 임시 입력으로만 사용한다. 실패 시 `UNKNOWN`으로 수렴한다. |
| 2.3 Bucket Rollup Service | Ready after 2.2 | UTC 30초 boundary, `[start, end)`, cumulative histogram, normalized route-only input이 정의되어 있다. | starter는 `localPercentiles`로 canonical p95/p99를 보고할 수 있지만 state/rule 계산은 하지 않는다. |
...
- `planning-artifacts/sprint-change-proposal-2026-05-14.md` 기준 Route Attribution B안: Story 2.1/2.2/2.3, contract, architecture, sprint plan, acceptance traceability에 `http.route` 우선, raw path candidate allowlist 일시 입력, query 폐기, exact-one allowlist match, `UNKNOWN` 수렴 정책을 반영했다.
