...
6. heartbeat telemetry를 표시하더라도 starter connection status와 accepted bucket freshness/application state를 분리한다. 최근 heartbeat와 오래된 accepted bucket 조합은 no recent traffic, waiting for traffic, metric data idle 계열로 표현하고 host application down으로 단정하지 않는다.
