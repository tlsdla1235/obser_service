...
7. UI는 read model을 표시한다. endpoint ranking, app state, p95/p99를 화면에서 재계산하지 않는다.
...
| `IngestHeartbeatService` | project key와 metadata shape를 검증하고 lightweight heartbeat telemetry를 기록. bucket/snapshot/event/read-model 계산을 호출하지 않음 |
...
| `HeartbeatTelemetryRepository` 후보 | starter heartbeat의 `lastHeartbeatAt`, `lastHeartbeatStatus`, failure category 저장/조회. application state 계산에는 사용하지 않음 |
...
10. `DashboardReadModelService`는 dashboard query 또는 scheduled snapshot 시 accepted bucket을 source-of-truth로 bucket distribution, state, triage, endpoint priority를 구성하고, p95/p99는 starter canonical percentile을 사용한다.
...
6. heartbeat 성공은 accepted bucket, application/instance catalog row, dashboard snapshot, operational event, state/read-model calculation을 만들지 않는다.
7. heartbeat 미수신은 host application down 판정이 아니며 starter connection 상태로만 표현한다.

### 6.2 Read Flow

1. UI가 dashboard snapshot을 요청한다.
2. `DashboardController`는 path variable을 query DTO로 변환한다.
3. `DashboardReadModelService`는 current 15분 window와 baseline 15분 window 기준 accepted bucket을 읽고, 필요하면 저장된 snapshot 후보를 조회하거나 생성한다.
...
6. heartbeat telemetry를 표시하더라도 starter connection status와 accepted bucket freshness/application state를 분리한다.
...
- 의미: metric ingest가 아닌 periodic control-plane/liveness signal
- side effect: accepted bucket, catalog upsert, dashboard snapshot, operational event, state/read-model calculation 없음
...
- Heartbeat contract test: valid heartbeat returns received status without bucket/snapshot/event/read-model side effects.
