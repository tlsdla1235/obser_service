...
| D7 | Histogram boundary set cross-check | 각 histogram의 positive `leMs`, cumulative count, count upper bound를 service에서 검증한다. boundary set 전체 일치 검증은 보류한다. | starter bug로 summary와 endpoint boundary set이 달라지면 bucket distribution 표시와 진단 해석이 부정확해질 수 있다. | dashboard bucket distribution read model 구현 전 |
