...
아래 표는 Epic 13 시작 시점의 gap map이다. 현재 상태를 재정의하는 표가 아니라, 완료된 alignment story가 어떤 해석을 좁혔는지 추적하기 위해 보존한다.
...
| Snapshot scheduler | scheduler는 legacy scheduled token과 1시간 target span 해석에 기대고 있었다. | 사용자-facing scheduled snapshot UX는 30분 slot이다. Persisted token은 legacy persistence/API token으로만 유지한다. | scheduler slot을 30분으로 바꾸되 token rename은 하지 않는다. |
...
| Instance Evidence | live evidence에 legacy 15분 naming과 shape가 남아 있었다. | Instance Dashboard는 Application Dashboard와 같은 30분 window의 selected instance evidence detail이다. | live instance evidence를 30분으로 정렬한 뒤 snapshot mode를 추가한다. |
...
- 2026-06-10: Story `13-5-frontend-application-dashboard-ia-realignment`에서 canonical `dashboard_read_model.v1` 기반 Application Dashboard live IA, adapter/type/guard fixture를 구현하고 done 상태로 닫았다.
...
- 2026-06-10: Story `13-7-frontend-snapshot-history-detail-realignment`에서 snapshot detail을 stored dashboard read model 복원 surface로 정렬하고 expired/404 safe copy guard를 보강했다.
...
- 2026-06-10: Story `13-7-frontend-snapshot-history-detail-realignment`에서 marker-first 30분 history/date/slot 탐색, 14일 672-slot horizon, marker/state divergence guard를 구현했다.
...
- 2026-06-11: Story `13-9-frontend-instance-surface-split`에서 live Instance Dashboard detail, selected Application Snapshot 기준 snapshot detail, stored Instance Snapshot Trend surface를 분리하고 done 상태로 닫았다.
...
- 2026-06-10: Story `13-3-backend-recent-30-minutes-window-alignment`에서 Application Dashboard와 Instance evidence live window를 `recent_30_minutes` accepted bucket source로 정렬했다.
...
- 2026-06-10: Story `13-4-backend-application-dashboard-read-model-shape-alignment`에서 live response와 snapshot stored payload가 공유하는 canonical `dashboard_read_model.v1` shape를 구현했다.
...
- 2026-06-10: Story `13-6-backend-30-minute-scheduled-snapshot-and-slot-horizon-alignment`에서 30분 scheduled snapshot cadence, legacy persisted token guard, `current_window_end_utc` horizon/order alignment를 완료했다.
...
- 2026-06-10: Story `13-6`과 `13-7`에서 marker/history/trend slot horizon과 stored snapshot detail source guard를 정렬했다.
...
- 2026-06-10: Story `13-8-backend-instance-dashboard-live-snapshot-mode-split`에서 live/snapshot mode API/service/read model 책임을 분리하고 done 상태로 닫았다.
...
- 2026-06-11: Story `13-10-retention-cleanup-alignment`에서 `RetentionCleanupProperties`, `RetentionCleanupService`, `RetentionCleanupScheduler`, `RetentionCleanupResult`, repository bulk delete, read-surface retention guard를 구현했다.
- Cleanup physical delete rollout은 기본 disabled이고, enabled/dry-run은 schedule/cutoff 의미를 바꾸지 않는 운영 제어로만 둔다.
