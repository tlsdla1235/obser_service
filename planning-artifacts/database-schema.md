...
PostgreSQL을 범용 TSDB나 rule engine으로 쓰지 않는다. lifecycle state, insight rule ranking, endpoint priority, p95 계산의 source of truth는 portal domain/application이다.
...
endpoint별 데이터는 `accepted_metric_buckets.endpoints_json`에 bounded JSON으로 저장한다. MVP에서 endpoint 수는 contract상 bounded top-N 또는 allow set이므로 별도 endpoint table 없이도 15분 current/baseline window를 application에서 병합할 수 있다.
...
- `duration_buckets_json`과 `endpoints_json`의 histogram monotonicity, boundary set 일치 여부는 DB check가 아니라 `AcceptIngestEnvelopeUseCase`에서 검증한다.
- p95는 이 테이블에서 SQL view로 계산하지 않는다. application이 window bucket을 읽어 `histogram-merge` contract에 따라 계산한다.
...
- `read_model_json`은 UI response source로 저장하되, 계산 source는 application service다.
...
- portal 내부 scheduled cleanup adapter가 retention 기준보다 오래된 bucket/snapshot을 삭제한다.
...
DB는 window bucket을 효율적으로 읽고 snapshot을 저장하는 adapter 역할만 맡는다.
...
6. `V006__retention_cleanup_support.sql`
   ...
   - 별도 stored procedure는 만들지 않고 application scheduled cleanup에서 사용
...
- dashboard snapshot refresh는 ingest transaction commit 후 portal in-process 작업으로 수행한다.
- dashboard query 시점에 snapshot이 없거나 명백히 오래된 경우 `QueryDashboardSnapshotUseCase`가 fallback으로 재생성할 수 있다.
...
comment on column dashboard_snapshots.generated_at is 'portal application이 read model을 생성한 시각.';
...
- Epic 5에서는 `dashboard_snapshots`와 read model snapshot 저장/조회 adapter를 구현한다.
