...
| `starter_credential_status` | `varchar(32)` | no | starter credential 상태. `active`, `revoked` |
| `starter_credential_issued_at` | `timestamptz` | no | 현재 starter credential metadata 최초 발급 시각 |
| `starter_credential_rotated_at` | `timestamptz` | yes | 마지막 rotation 시각. 아직 없으면 null |
| `starter_credential_revoked_at` | `timestamptz` | yes | credential 폐기 시각. active 상태이면 null |
...
- Check: `ck_projects_starter_credential_status (starter_credential_status in ('active', 'revoked'))`

Notes:

- `projects.status`는 project visibility/lifecycle 상태이고 `starter_credential_status`는 ingest credential lifecycle 상태다. Project를 숨기지 않고 starter credential만 폐기할 수 있도록 두 상태를 섞지 않는다.
- Raw starter credential은 DB row, migration, repository lookup surface에 저장하지 않는다. Create/rotate 성공 response의 1회 표시 field에서만 허용한다.
- Rotation은 같은 row의 `key_prefix`/`project_key_hash`를 새 credential metadata로 교체하고 `starter_credential_rotated_at`을 갱신한다. Grace period나 multi-active credential은 MVP schema에 없다.
- Revocation은 `starter_credential_status='revoked'`와 `starter_credential_revoked_at`만으로 표현한다. Project list visibility는 `projects.status`와 account-project membership으로 계속 결정한다.
