...
이 story는 starter가 주기적으로 portal 도달 가능성, project key 유효성, `application/environment/instance` metadata shape, starter liveness를 알리는 heartbeat 계약을 고정한다. 또한 이후 project -> application -> instance 탐색에서 app/project/window rollup은 서버 histogram merge로 유지하되, 개별 instance 화면에는 starter가 직접 계측한 30초 bucket p95/p99 추세와 대표값을 표시할 수 있게 계약을 고정한다.
...
- `localPercentiles` ingest payload 후보와 non-mergeable 의미 정의
- server-side histogram merge로 application/endpoint/window canonical p95/p99 계산
- 개별 instance detail에서 starter-local 30초 bucket p95/p99의 평균, 중앙값, 최솟값, 최댓값을 명확한 라벨로 강조해서 보여주는 read model 후보
- sample guard 통과 시 p99를 auxiliary evidence로만 사용하는 경계
...
- app/project rollup을 starter-local p95/p99 값으로 만드는 것
- 30초 bucket p95/p99 요약값을 canonical 15분 p95/p99 또는 application/project p95/p99라고 부르는 것
...
개별 instance에 대해서는 starter가 각 30초 bucket에서 직접 계측한 p95/p99를 보낼 수 있다. 이 값은 app/project/window rollup source가 아니라 instance detail과 snapshot에서 쓰는 **instance-local bucket percentile evidence**다.

이 story의 정렬 기준은 `localPercentiles`를 제거하거나 숨기는 것이 아니다. 운영자는 instance별 tail latency를 구체적인 숫자로 보고 싶어 하므로, instance detail에서는 이 값을 충분히 강조할 수 있다. 다만 강조되는 값의 의미는 "n분 전체 요청의 p95/p99"가 아니라 "n분 동안 생성된 30초 bucket p95/p99 값들의 요약 통계"로 고정한다.
...
- `localPercentiles.p95Ms`와 `localPercentiles.p99Ms`는 해당 instance의 해당 30초 bucket에 대한 starter-local observed value다.
- `localPercentileSummary.p95.avgMs`와 `p99.avgMs`는 canonical n분 p95/p99가 아니다. 해당 window 안의 30초 bucket percentile 값들에 대한 평균이다.
...
- current, baseline, snapshot window에 각각 같은 summary를 만들 수 있다.
- app/project p95/p99, canonical 15분 window p95/p99, rule source-of-truth는 histogram merge 결과를 유지한다.
- UI label은 “instance-local 30초 bucket p95/p99 요약”, “bucket p99 평균”, “bucket p99 중앙값”, “bucket p99 최댓값”처럼 의미를 드러내야 한다.
- 피해야 할 라벨은 “15분 p99”, “current p99”, “application p99”, “canonical p99”, “서비스 p99”다.
- Instance detail에서는 이 요약을 headline으로 강조할 수 있다. Application/project home, state banner, freshness indicator, rule reason, endpoint priority에서는 이 값을 source-of-truth처럼 표시하지 않는다.
...
- 후속 snapshot story는 저장 당시 read model, instance-local percentile summary, histogram-derived bounded tail evidence를 분리해서 정의한다.
...
12. ingest envelope는 `localPercentiles` 후보를 instance-local 30초 bucket trend field로 문서화하며, `scope=instance_bucket`, `source=starter_local`, `mergeable=false` 의미를 고정한다.
13. instance detail read model은 starter-local p95/p99 trend와 current/baseline n분 window의 bucket percentile avg/median/min/max headline을 표시할 수 있다.
14. app/project p95/p99, canonical 15분 window p95/p99, rule source-of-truth는 server-side histogram merge 결과로만 표현한다.
15. p99는 app/project 판단에서는 충분한 sample guard를 통과한 auxiliary evidence이며, degraded/down 판단이나 app/project rollup의 단독 근거가 될 수 없다.
16. UI는 heartbeat result, state, p95/p99, endpoint priority를 직접 계산하지 않고 server response/read model을 표시한다.
17. Story 4-0은 contract gate이며 project -> application -> instance navigation, heartbeat endpoint 구현, instance detail API 구현, snapshot marker 구현은 후속 story로 분리한다.
18. 후속 snapshot contract는 단일 long-window p99를 만들지 않으며, instance-local percentile summary와 histogram-derived bounded evidence의 세부 shape는 별도 story에서 확정한다.
19. heartbeat 미수신은 host application down 판정 근거가 아니며, starter connection 상태와 accepted bucket 기반 operational state를 분리해 표현한다.
...
- `histogram-merge.md`가 app/project/window canonical percentile source-of-truth다.
- 개별 instance의 30초 p95/p99는 starter-local trend evidence로 받을 수 있다.
- `localPercentileSummary.avgMs`는 n분 p95/p99가 아니라 30초 bucket percentile 값들의 평균이다.
- `localPercentileSummary.medianMs`, `minMs`, `maxMs`도 30초 bucket percentile point 집합의 요약값이며, instance detail에서 구체적인 tail latency evidence로 강조할 수 있다.
- 사용자 제시 옵션 기준 권장안은 histogram 유지 + starter-local percentile trend 병행이다.
- p95/p99 scalar만으로 histogram을 대체하는 방향은 app/project/time-window rollup을 통계적으로 오도할 위험이 커서 reject한다.
- p95/p99 값만으로 app/project rollup을 만들지 않는다.
- 후속 snapshot story에서 canonical p99를 저장할 때는 값끼리 평균/최댓값 조합으로 만들지 않는다. subwindow histogram merge 결과 또는 representative bucket 값을 bounded evidence로 저장한다.
- 후속 snapshot story는 instance-local percentile summary와 histogram-derived bounded tail evidence의 shape를 별도로 확정한다.
...
- `HistogramMergeService` 후속 test guard
  ...
  - merged application p95는 histogram count merge에서 계산
  - `localPercentiles` payload가 schema validation에서 거절되지 않음
  - `localPercentiles` payload가 app/project/window rollup input으로 쓰이지 않음
  - p99 insufficient sample guard
  - `localPercentileSummary`의 avg/median/min/max가 30초 bucket local percentile points에서 계산됨
  - `localPercentileSummary.avgMs`를 canonical 15분 p95/p99로 노출하지 않음
  - snapshot `trendSlices` 후보는 subwindow histogram merge 결과로 계산
...
- p95/p99 값만으로 app/project rollup을 만들지 않는다.
- `localPercentileSummary.avgMs`를 canonical 15분 p95/p99라고 부르지 않는다.
- `localPercentileSummary.avgMs/medianMs/minMs/maxMs`는 instance detail의 bucket percentile summary로만 표시한다.
