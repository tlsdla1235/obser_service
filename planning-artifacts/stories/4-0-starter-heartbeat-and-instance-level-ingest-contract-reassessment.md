...
## Portal Ingest Validation Review Scope Policy

Story 4.0은 portal ingest validation을 다시 열되, MVP 진행을 막는 추가 hardening review가 되지 않도록 아래 범위로 제한한다.

- Story 4.0 이후 리뷰/검토에서 portal ingest validation을 다시 열 때도, 기존 ingest envelope field들에 대해서는 추가 안정성 검증을 새로 요구하지 않는다.
- 기존 field란 Story 3.x에서 이미 구현/수용된 `schemaVersion`, `application`, `bucket`, `summary.requestCount/errorCount`, `summary.httpServerDurationBuckets`, `summary.jvm`, `summary.datasource`, `endpoints`, `idempotencyKey` 계열을 말한다.
- 이 기존 field들은 현재 MVP acceptance 수준을 baseline으로 삼고, edge-case review가 추가 hardening을 발견하더라도 Story 4.0 또는 다음 sprint planning의 blocker로 삼지 않는다.
- 검증 검토 대상은 Story 4.0으로 의미가 바뀌거나 새로 추가되는 field/contract에 한정한다.
  - 예: `summary.localPercentiles`
  - 예: heartbeat request/response 후보
  - 예: source/scope/display policy enum이나 read model shape
- 수정/추가 field에 대해서도 검증은 “저장 가능성, source/scope 의미 보존, 보안/권한, null/필수값, 기존 read model 오해 방지” 수준으로 제한한다.
- 수정/추가 field 검증을 이유로 기존 histogram, endpoint, route, count, idempotency, runtime ratio에 대한 추가 검증을 재개하지 않는다.
...
- Story 4.0 구현에서 portal ingest validation을 수정할 때는 `summary.localPercentiles` 같은 신규/변경 field만 다룬다. 기존 `schemaVersion`, bucket, histogram, endpoint, count, idempotency, runtime ratio 검증의 안정성 강화는 deferred hardening으로만 기록한다.
...
- `HistogramMergeService` 후속 test guard
  ...
  - Story 4.0 localPercentiles 검증 테스트가 기존 histogram, endpoint, route, count, idempotency, runtime ratio 검증을 새 요구사항으로 확장하지 않음
...
- [x] `summary.localPercentiles`를 portal ingest request model, validation, accepted bucket persistence 경로에 optional supported field로 추가한다.
- [x] 신규 field 검증을 source/scope, bucket boundary 일치, required/null, 저장 가능한 primitive value, `mergeable=false` 의미 보존 수준으로 제한한다.
- [x] 기존 ingest envelope field들에 대한 추가 안정성 검증 재개 금지 정책을 Story 4.0과 ingest contract에 명시한다.
- [x] 신규 field acceptance/rejection/persistence test를 추가하고 기존 field hardening을 새 요구사항으로 확장하지 않는다.

## Dev Agent Record

### Debug Log

- 2026-05-22: Story 4.0이 contract gate임을 확인하고 heartbeat endpoint/UI/snapshot 구현은 범위 밖으로 유지했다.
- 2026-05-22: portal ingest validation은 `summary.localPercentiles` 신규 field만 추가 검증하고 기존 Story 3.x field 검증은 변경하지 않았다.

### Completion Notes

- `summary.localPercentiles`를 optional supported field로 파싱, 제한 검증, accepted bucket 저장 JSON에 보존하도록 구현했다.
- 검증 범위는 `scope=instance_bucket`, `source=starter_local`, envelope bucket boundary 일치, required numeric values, `p99Ms >= p95Ms`, `mergeable=false`로 제한했다.
- 기존 histogram, endpoint, route, count, idempotency, runtime ratio 안정성 hardening은 Story 4.0 blocker로 다루지 않는 정책을 문서화했다.
...
- `observability-portal/src/main/resources/db/migration/V004__add_local_percentiles_to_accepted_metric_buckets.sql`
...
- 2026-05-22: Story 4.0 localPercentiles ingest validation/persistence와 validation review scope policy를 추가했다.
