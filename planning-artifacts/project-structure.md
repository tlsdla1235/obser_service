...
Epic 5/6 dashboard 구현은 Project Entry, Application List, Application Dashboard, Instance Detail, Instance Snapshot Trend, Snapshot/History 화면을 같은 portal runtime 안에서 제공한다. Project/Application navigation은 catalog/dashboard read model 책임이고, instance trend/history는 stored dashboard snapshot/read model projection 책임이다.
...
Epic 6 UI는 `planning-artifacts/prototypes/epic5-6-dashboard-flow-prototype.html`의 baseline 화면 순서를 따른다.
...
Instance Snapshot Trend와 Snapshot/History UI는 stored dashboard snapshot/read model projection/marker를 표시하며 raw explorer나 arbitrary query UI가 아니다.
