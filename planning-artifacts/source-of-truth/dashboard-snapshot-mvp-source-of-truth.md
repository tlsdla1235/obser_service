...
2026-06-11 UI MVP 결정: 과거 instance evidence는 별도 Stored trend/projection trend 화면이 아니라 Snapshot/History에서 snapshot을 선택한 뒤 snapshot-mode wide modal로 확인한다. Instance Snapshot Trend, InstanceTrendView, narrow Sheet, `openTrend`/`openLiveDashboard`, `snapshotTrend` surface는 현재 MVP snapshot surface가 아니다.

2026-06-12 구현 결정(snapshot-mode = live surface 재사용):
- Snapshot/History에서 slot을 클릭하면 별도 detail 화면으로 redirect하지 않고 같은 자리에서 mode=snapshot으로 전환한 뒤 상단으로 scroll하고, live와 **동일한 dashboard 컴포넌트 트리**를 그대로 렌더한다. 상단에 snapshot-mode banner와 "라이브로 돌아가기"를 둔다.
- 데이터 원천은 bounded snapshot detail이 아니라 capture 당시 저장된 **full `read_model_json`**이다. 진입 API는 `GET .../dashboard/snapshots/{snapshotId}/read-model`이고, 저장된 JSON을 가공/재계산 없이 `mode=snapshot`으로 그대로 반환한다(record round-trip 없이 raw passthrough).
- 저장본은 immutable historical artifact다. `InstanceEntry.summary`(D5) 이전에 저장된 snapshot은 `instances[]`에 `summary`가 없을 수 있으며, 이를 backfill하거나 현재 metric으로 재계산하지 않는다. snapshot-mode guard는 summary 부재를 core field만으로 허용하고, instance 행은 "이 시점 snapshot에는 instance summary가 저장되지 않았습니다"로 표시한다.
...
새 operational event table, endpoint timeseries table, raw snapshot explorer는 MVP 범위가 아니다. Instance Snapshot Trend/Stored trend/projection trend UI surface도 현재 MVP 범위가 아니며, 저장 projection 계약은 Post-MVP 후보 또는 backward compatibility 설명으로만 남긴다.
...
7. Instance Snapshot Trend UI 재도입 여부
