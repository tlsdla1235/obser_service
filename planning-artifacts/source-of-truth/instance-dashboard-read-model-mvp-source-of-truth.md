...
Instance trend API가 필요하면 기존처럼 stored `instanceSummary.items[]` projection으로 제한한다. Trend는 current state나 health score를 재계산하지 않는다. Instance Dashboard snapshot mode와 Instance Snapshot Trend는 서로 다른 read surface다.
...
| `dashboard_snapshots.read_model_json.instanceSummary.items[]` | instance trend와 snapshot summary projection에는 적합. Instance Dashboard detail의 필수 source는 아님 |
...
- `items[]` 최대 50개를 유지한다.
- `instanceSummary.items[]` cap은 Instance Snapshot Trend와 snapshot summary 노출 범위의 제한으로만 본다.
...
12. 기존 `instanceSummary.items[]` minimum shape는 Instance Snapshot Trend와 snapshot summary를 위해 유지하되, Instance Dashboard snapshot mode의 필수 source로 만들지 않는다.
