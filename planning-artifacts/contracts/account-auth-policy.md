...
기본 browser-facing GitHub OAuth callback은 token pair JSON을 화면에 렌더링하지 않는다. `/api/auth/github/callback`은 no-store HTML relay page를 반환하고,
dashboard popup opener가 1회용 server-memory relay를 통해 service access token만 받아 in-memory auth state로 세팅한다.
이 relay는 service token을 URL query/hash/fragment, cookie, `localStorage`, `sessionStorage`에 넣지 않기 위한 짧은 수명 one-time handoff이며,
refresh token은 기본 dashboard flow에 전달하지 않는다.

Token pair JSON이 필요한 자동화/테스트 도구는 browser callback과 분리된 explicit API endpoint를 사용한다. `/token/refresh` response는 기존처럼 우리 서비스 access token/refresh token을 JSON response body로 전달한다.
Cookie 기반 server session, redirect fragment token 전달, browser local storage 저장 전제는 여전히 Story 6.1 계약에 포함하지 않는다. Provider token/raw payload/secret은 response/log/error surface에 노출하지 않는다.
...
- Story 9.2부터 `/api/projects/{projectId}/starter-credential`, `/api/projects/{projectId}/starter-credential/rotations`, `/api/projects/{projectId}/starter-credential/revocations`도 같은 Bearer account + active account-project membership 경계 안에 둔다.
...
- Starter credential lifecycle API의 membership mismatch도 resource lookup 전에 body 없는 `404`로 닫고, project name, credential prefix/status, snapshot/history 존재 여부를 노출하지 않는다.
...
- Public project registration과 starter credential create/rotation/revocation은 account service token 인증과 starter ingest credential 인증을 분리해 다룬다. Invite/team/org/admin UI, billing/tenant model은 이 정책에서 열지 않는다.
...
5. Given browser-facing GitHub OAuth callback을 설계할 때, Then raw service access/refresh token JSON은 화면에 렌더링하지 않고 cookie server session, URL token 전달, browser storage 저장을 사용하지 않는다.
