...
Recovery는 이전 metric state가 `stale` 또는 `down`이었고, 현재 accepted bucket freshness가 `current`로 회복됐지만 최소 sample 조건이 아직 부족한 경우에만 활성화한다. 즉 `previousState in [stale, down]` + `freshness=current` + `sampleReadiness=insufficient` 조합이면 현재 metric state는 `unknown`으로 두고 read model의 `recovery.isRecovering=true`로 표현한다.

`waiting_first_data -> unknown`은 recovery가 아니다. `degraded -> active` 또는 기존 degraded 해소는 Story 4.2의 degraded hysteresis 범위이며, Story 4.3 recovery guidance와 섞지 않는다.

Recovery trigger가 더 이상 성립하지 않으면 `isRecovering=false`다. 특히 freshness가 `current`이고 sample이 sufficient가 되면 recovery는 종료된다. freshness가 다시 stale/down 후보가 되면 recovery가 아니라 stale/down metric state로 표현한다.

MVP UI state enum에는 별도 최상위 `recovering` state를 늘리지 않고, recovery 안내 문구로 처리한다.
...
`lastHealthyAt`은 현재 요청에서 추론하지 않고 이전 healthy/active read model 또는 snapshot에서 받은 값만 사용한다. 값이 없으면 `null`이다. `lastRecoveredAt`은 회복 완료 시각처럼 읽히므로 Story 4.3 계약에 포함하지 않는다.

`retryAfterSeconds`는 `isRecovering=true`일 때 30초로 고정한다. 이는 30초 bucket/drain cadence에 맞춘 다음 판단 대기 힌트이며 자동 재시도 약속이 아니다. Recovery가 아니면 `null`이다.

Recovery 활성화와 triage card 없음 조합은 zero-insight reason `observing_recovery`로 표현한다. `no_recent_traffic`은 starter connection diagnosis로 유지할 수 있지만 MVP zero-insight reason code로 새로 추가하지 않고 `metric_data_idle`로 수렴한다.

Metric recovery guidance와 starter connection guidance는 별도 copy surface로 유지한다. 두 문구를 결합해 host application down, host process down, 앱 내려감 같은 확정 표현을 만들지 않는다.
