...
State와 freshness의 source-of-truth는 accepted bucket뿐이다. Starter heartbeat는 별도의 control-plane/liveness signal이며, heartbeat 성공 또는 미수신은 `waiting_first_data`, `stale`, `down`, `degraded`를 직접 만들거나 해소하지 않는다.
...
| `down` | 최근 accepted bucket이 180초 이상 없다 | host application down으로 단정하지 않고 accepted bucket 수집 경로와 starter 설정 확인을 우선 제안한다 |
...
heartbeat telemetry가 존재하더라도 evaluation order에는 들어가지 않는다. UI가 heartbeat를 함께 표시할 때도 starter 연결 상태와 accepted bucket 기반 application state는 별도 축으로 유지한다.
...
Heartbeat success/failure/missing은 이 state transition 후보가 아니다. heartbeat telemetry는 별도 starter connection surface에서 다루며 operational event history를 생성하지 않는다.
...
- heartbeat service나 repository 후보는 application state를 계산하거나 갱신하지 않는다.
