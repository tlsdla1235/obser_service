...
Starter heartbeat는 operational event source가 아니다. Heartbeat 성공, 실패, 미수신은 accepted bucket, dashboard snapshot, operational event, p95/p99, rule/metric read-model calculation을 생성하거나 암시하지 않는다. Starter connection/liveness를 보여주는 surface가 필요하면 accepted bucket 기반 operational event history와 분리된 heartbeat read model에서 다룬다.
