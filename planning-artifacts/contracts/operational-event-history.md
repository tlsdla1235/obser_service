...
| dashboard snapshot | `dashboard_snapshots`에 저장된 특정 시점의 dashboard read model 전체 |
...
`dashboard_snapshots`는 spike 전용 table이 아니라 dashboard read model history 저장소다. `read_model_json`에는 해당 시점의 state, p95, triage cards, endpoint priority, bounded endpoint evidence를 포함한다.
...
Snapshot `read_model_json`에 남기는 endpoint evidence는 최대 `10개`다. 우선순위는 top triage card에 연결된 endpoint, `endpointPriority` 상위 항목, high-confidence concern endpoint 순서다. Evidence에는 `method`, `route`, `endpointKey`, `rank`, `reason`, `ruleIds`, `confidence`, `requestCount`, `errorRate`, `p95Ms`, `baselineP95Ms`, guard 통과 시 `p99Ms`, `tailLatencyEvidence`, `freshness`, `recommendedAction`만 담는다. raw path, query string, query key/value, trace id, per-request sample은 담지 않는다.
...
## 10. p99 and Tail Latency Evidence Rules
