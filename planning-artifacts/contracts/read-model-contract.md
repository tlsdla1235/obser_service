...
status: aligned-with-dashboard-source-of-truth
date: 2026-06-09
dependsOn:
...
`read-model-contract`는 first-screen UI 구현 계약이다.

최신 제품 판단은 `planning-artifacts/source-of-truth/*source-of-truth.md` 문서가 우선한다. 이 문서는 해당 Source of Truth와 충돌하지 않도록 MVC response shape와 persistence boundary를 보조로 설명한다.
...
Dashboard current response는 query 시점 기준 `최근 30분` accepted metric bucket을 해석해 만든 현재 상태 read model이다. MVP 판단은 baseline 비교를 사용하지 않는다. `state`, 기본 `metrics`, source-scoped starter percentile, `triageCards`, `endpointPriority`는 UI가 계산하지 않고 `DashboardReadModelService`와 하위 service 결과를 그대로 표시한다.
...
`dashboard_snapshots`는 spike 전용 저장소가 아니라 특정 시점의 dashboard read model 전체를 저장하는 coarse-grained history 저장소다. MVP 기본 저장 cadence는 application별 `30분 scheduled snapshot slot`이며, ingest로 수용된 30초 bucket마다 dashboard snapshot을 만들지 않는다.
...
`accepted_metric_buckets`는 30초 단위 계산 원천이고 장기 analytics store가 아닌 bounded operational retention source다. MVP에서는 dashboard snapshot UI와 Instance Dashboard snapshot mode를 맞추기 위해 사용자-facing retention horizon을 14일로 둔다. `dashboard_snapshots.read_model_json`은 그 시점에 UI가 소비한 bounded endpoint summary, evidence, `endpointPriority`를 포함할 수 있다.

### 1.2 Snapshot Capture, Retention, and Evidence Boundary

`dashboard_snapshots` 기본 retention은 `14일`이다. 이 값은 운영 비용과 demo 요구에 맞춰 config로 조정 가능해야 한다. MVP에서는 `accepted_metric_buckets`도 같은 14일 사용자-facing horizon을 따르며, 가장 오래된 retained 30분 snapshot window를 보존하기 위해 physical metric cleanup에는 30분 evidence grace를 둘 수 있다.

기본 저장은 application별 `30분 scheduled snapshot slot`이다. 추가 snapshot row는 아래 조건에서만 남긴다.
...
이 capture 정책은 30초 dashboard snapshot 장기 보관이 아니다. 사용자가 늦게 들어왔을 때 최근 운영 흐름을 30분 dashboard point 단위로 이해할 수 있도록 coarse-grained read model history를 보강하는 bounded capture다.
...
Snapshot `read_model_json` 안에 남기는 endpoint evidence는 최대 `10개`다. 우선순위는 top triage card에 연결된 endpoint, `endpointPriority` 상위 항목, high-confidence concern endpoint 순서다. Endpoint evidence field 후보는 `method`, `route`, `endpointKey`, `rank`, `reason`, `ruleIds`, `confidence`, `requestCount`, `errorRate`, `durationBuckets`, `bucketDistributionSource`, `freshness`, `recommendedAction`이다. raw path, query string, query key/value, trace id, per-request sample, endpoint p95/p99는 포함하지 않는다.
  ...
  "triageCards": [
    {
      ...
      "summary": "최근 30분 오류율이 운영 기준을 넘었습니다.",
...
기본 조회 horizon은 `7일`이고, 선택 가능한 최대 horizon은 `dashboard_snapshots` retention 안에서 `14일`로 clamp한다. 기본 limit은 30분 scheduled snapshot 기준 `336` point이고, 최대 limit은 `672` point다. 누락된 snapshot은 보간하지 않고 누락으로 둔다.
...
History service는 retention horizon과 30분 slot 선택에는 `current_window_end_utc`를 우선 사용하고, capturedAt/provenance와 deterministic ordering 보조값으로 `generated_at`을 사용한다. Detail source는 계속 `read_model_json`이다. 필요한 경우 `capture_reason`, `primary_rule_id`, `primary_endpoint_key`, `max_confidence` 같은 bounded index/search helper column을 사용할 수 있다. 그 column은 저장된 read model의 검색 편의를 위한 복사 값이어야 하며 raw metric이나 별도 endpoint timeseries를 대체 저장하는 용도가 아니다.
