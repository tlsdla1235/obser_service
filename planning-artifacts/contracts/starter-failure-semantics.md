...
이 계약은 starter에서 portal로 bucket/envelope를 전송하는 경로와 starter heartbeat를 보내는 경로에 적용한다. Bucket ingest는 metric data plane이고, heartbeat는 periodic control-plane/liveness signal이다.
...
Starter heartbeat도 같은 fail-open 원칙을 따른다. heartbeat 실패, timeout, 미수신은 host application down 판정이 아니며, dashboard application freshness/state의 source-of-truth도 아니다. heartbeat 성공은 accepted bucket, dashboard snapshot, operational event, state/read-model calculation을 만들거나 암시하지 않는다.

UI가 heartbeat telemetry를 보여줄 경우에는 `starter heartbeat/연결 상태`를 `accepted bucket freshness/application state`와 분리해서 표현한다. 예를 들어 heartbeat가 최근 수신됐더라도 accepted bucket이 없으면 application state는 계속 `waiting_first_data`일 수 있다.
...
- heartbeat 미수신이 host application down 판정으로 변환되지 않는 test
...
- heartbeat를 metric ingest 성공이나 host application health 판정으로 사용하는 정책
