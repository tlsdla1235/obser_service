...
1. framework가 제공한 `http.route` 또는 route template이 있고 blank/`UNKNOWN`이 아니면 low-cardinality route candidate로 최우선 사용한다.
2. framework route candidate에 query string이 붙어 있으면 starter는 `?` 이후를 폐기한 뒤 route template contract를 검증할 수 있다.
3. query 폐기 후 route가 portal final payload route contract를 만족하면 그 값을 normalized route로 사용한다.
4. query 폐기 후에도 absolute URL, slash 없는 path, trailing slash, double slash, raw identifier candidate(`/orders/12345`, `/assets/deadbeef`), malformed template(`/orders/{order-id}`, `/orders/{orderId`, wildcard 등), invalid percent encoding 등 invalid shape이면 allowlist로 우회하지 않고 `UNKNOWN`으로 수렴한다.
5. `http.route`가 없거나 blank/`UNKNOWN`일 때만 raw path candidate를 query 폐기 후 allowlist matcher의 임시 입력으로 사용한다.
6. raw path candidate가 configured allowlist template과 정확히 하나만 매칭되면 해당 configured template을 사용한다.
7. 그 외 모든 경우 `UNKNOWN`을 사용한다.

`http.route`가 present인 경우에는 raw path fallback을 적용하지 않는다. query string이 있다는 사실만으로 framework route candidate가 invalid인 것은 아니지만, query 폐기 후에도 final route contract를 만족하지 못하면 `UNKNOWN`이다.

raw path fallback은 `http.route`가 비어 있거나 명시적으로 `UNKNOWN`인 경우에만 가능하다. raw path candidate에서도 query string은 폐기할 수 있지만, 최종 route는 raw path가 아니라 configured allowlist template이어야 한다.

query 폐기는 starter-side normalization 정책이다. final ingest envelope에는 query string, raw path, query key/value, high-cardinality tag가 남으면 안 된다. Portal validation은 약화하지 않으며, final payload route에 query string이 있으면 여전히 reject한다. starter가 query를 폐기해 safe normalized template으로 만든 경우만 OK이고, final payload에 query가 남는 것은 NG다.
...
| `http.route=/orders/{orderId}?debug=true`, raw path `/orders/123?debug=true` | 없음 | `/orders/{orderId}` |
| `http.route=/orders/12345?debug=true`, raw path `/orders/12345?debug=true` | `/orders/{orderId}` | `UNKNOWN` |
| `http.route=/orders/?debug=true`, raw path `/orders/123` | `/orders/{orderId}` | `UNKNOWN` |
| `http.route=/assets/deadbeef?x=1`, raw path `/assets/deadbeef?x=1` | `/assets/{assetId}` | `UNKNOWN` |
...
- `RouteNormalizationService`: `http.route` 우선, framework route query 폐기 후 contract 검증, allowlist exact-one fallback, `UNKNOWN` 수렴 정책을 적용한다.
